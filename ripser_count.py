...
def barcode_square_sum(barcode, dim=0):
    """Calculate sum of squares of lengths of barcodes in h{dim}"""
    if(len(barcode[dim])):
        return np.sum(np.square((barcode[dim]['death'] - barcode[dim]['birth'])))
    else:
        return 0.0
...
def count_ripser_features(barcodes, feature_list=['h0_m']):
    ...
    for feature in feature_list:
        ...
        if ftype == 's':
            ...
            feat = [barcode_square_sum(barcode, dim) for barcode in barcodes]
